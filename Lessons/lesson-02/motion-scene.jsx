      ...
      {items.map((it, i) => {
        ...
        return (
          <div key={i} style={{
            ...
            <span style={{ font: `500 27px/1.34 ${SANS}`, color: INK, letterSpacing: '-0.011em' }}>
              {it.text}
            </span>
          </div>
        );
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, opacity: fo }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, top: 262, textAlign: 'center', opacity: tTitle.o, transform: `translateY(${tTitle.ty}px)` }}>
        <span style={{ font: `900 92px ${SANS}`, letterSpacing: '-0.035em', color: INK }}>Motion in a straight line</span>
      </div>
    ...
    <div style={{ position: 'absolute', inset: 0, opacity: fo }}>
      ...
      {dotIn > 0.4 && (
        <div style={{ position: 'absolute', left: dotPx, top: axisY - 116, transform: 'translateX(-50%)', opacity: clamp((lt - 1.6) / 0.5, 0, 1) * frameOpacity(lt, duration) }}>
          <div style={{ font: `600 30px ${SERIF}`, color: INK, whiteSpace: 'nowrap', textAlign: 'center' }}>
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, opacity: fo }}>
      ...
      <EqPanel x={364} y={150} lt={lt} from={5.3} fadeAt={11.0}>
        <div style={{ font: `400 40px ${SERIF}`, color: INK, whiteSpace: 'nowrap' }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, top: 150, opacity: win(lt, 15.5, 28, 0.5, 0.5) }}>
        <div style={{ display: 'flex', justifyContent: 'center', gap: 40 }}>
          <div style={{ borderLeft: `4px solid ${RED}`, padding: '14px 26px', background: '#FFFFFF' }}>
            ...
            <div style={{ font: `400 36px ${SERIF}`, color: INK, whiteSpace: 'nowrap' }}>Δ<I>x</I> = 4 − 2 = <span style={{ color: RED, fontWeight: 600 }}>+2 m</span></div>
          </div>
          <div style={{ borderLeft: `4px solid ${GRAY}`, padding: '14px 26px', background: '#FFFFFF' }}>
            ...
            <div style={{ font: `400 36px ${SERIF}`, color: INK, whiteSpace: 'nowrap' }}>5 + 3 = <span style={{ color: INK, fontWeight: 600 }}>8 m</span></div>
          </div>
        </div>
      </div>

      <Captions lt={lt} items={[
        { from: 1.0, to: 5.2, text: 'Displacement Δx is the change in position — final minus initial.' },
      ...
      <svg width="1280" height="720" style={{ position: 'absolute', inset: 0 }}>
        ...
        {showPts > 0 && (
          <g opacity={showPts}>
            ...
            <text x={tPx(t2) + 16} y={xPy(x2) - 10} fontFamily={SERIF} fontSize="18" fill={RED}>(t₂, x₂)</text>
          </g>
        )}
      </svg>
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, opacity: fo }}>
      ...
      <svg width="1280" height="720" style={{ position: 'absolute', inset: 0 }}>
        ...
        <text x={tPx(tP) - 16} y={xPy(xP) + 30} textAnchor="end" fontFamily={SERIF} fontSize="18" fill={INK} opacity={lt < 18 ? 1 : 0}>P (t = 3 s)</text>
      ...
      {!isTangentPhase && lt > 2.4 && (
        <div style={{ position: 'absolute', left: 250, top: 556, opacity: win(lt, 2.4, 9.2, 0.4, 0.25), display: 'flex', gap: 26, alignItems: 'center' }}>
          <div style={{ borderLeft: `4px solid ${GRAY}`, padding: '10px 22px', background: '#fff' }}>
            <span style={{ font: `400 30px ${SERIF}`, color: INK }}>Δ<I>t</I> = {f2(dt)} s</span>
          </div>
          <div style={{ borderLeft: `4px solid ${RED}`, padding: '10px 22px', background: '#fff' }}>
            <span style={{ font: `400 30px ${SERIF}`, color: INK }}>slope = </span>
            <span style={{ font: `600 32px ${SERIF}`, color: RED }}>{f2(secantSlope)} m/s</span>
          </div>
        </div>
      )}
      ...
      {isTangentPhase && (
        <div style={{ position: 'absolute', left: 250, top: 556, opacity: clamp((lt - 9.4) / 0.6, 0, 1), display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ borderLeft: `4px solid ${RED}`, padding: '12px 26px', background: '#fff', display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ font: `400 34px ${SERIF}`, color: INK }}><I>v</I> =</span>
            ...
            <Frac n={<span style={{ fontStyle: 'italic' }}>Δx</span>} d={<span style={{ fontStyle: 'italic' }}>Δt</span>} color={INK} />
            <span style={{ font: `400 34px ${SERIF}`, color: INK }}>=</span>
            <Frac n={<span style={{ fontStyle: 'italic' }}>dx</span>} d={<span style={{ fontStyle: 'italic' }}>dt</span>} color={INK} />
            {lt > 18 && <span style={{ font: `400 30px ${SERIF}`, color: INK }}>= 0.5<I>t</I></span>}
            <span style={{ font: `400 34px ${SERIF}`, color: INK }}>=&nbsp;</span>
            <span style={{ font: `600 34px ${SERIF}`, color: RED }}>{f2(tangentSlope)} m/s</span>
          </div>
        </div>
      )}
...
function SceneGraphRead() {
  ...
  // top: position
  const Tt = 80, Tb = 270; const xmin = -1, xmax = 9;
  const xpy = (v) => Tb - (v - xmin) / (xmax - xmin) * (Tb - Tt);
  // bottom: velocity
  const Vt = 342, Vb = 522; const vmin = -3, vmax = 3;
  ...
  // sweep builds v-t
  const sweep = interpolate([2.0, 11.0], [0, TM], [E.easeInOutSine])(lt);
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, opacity: fo }}>
      ...
      <svg width="1280" height="720" style={{ position: 'absolute', inset: 0 }}>
        ...
        {sweepFrac > 0.001 && <polyline points={ptsStr(vPts, tpx, vpy, sweepFrac)} fill="none" stroke={RED} strokeWidth="3.5" strokeLinecap="round" />}
        ...
        {sweepFrac > 0.001 && lt < 11.4 && <circle cx={tpx(curT)} cy={vpy(curV)} r="7" fill={RED} />}
      ...
      <div style={{ position: 'absolute', left: tpx(1.5), top: 60, transform: 'translateX(-50%)', opacity: ann(2.6, 5.4), textAlign: 'center' }}>
        <span style={{ font: `700 18px ${SANS}`, color: RED }}>steep ↑ — moving forward, fast</span>
      </div>
      <div style={{ position: 'absolute', left: tpx(5), top: 60, transform: 'translateX(-50%)', opacity: ann(5.6, 8.0), textAlign: 'center' }}>
        <span style={{ font: `700 18px ${SANS}`, color: INK }}>flat — momentarily at rest, v = 0</span>
      </div>
      <div style={{ position: 'absolute', left: tpx(8.5), top: 60, transform: 'translateX(-50%)', opacity: ann(8.2, 11.2), textAlign: 'center' }}>
        <span style={{ font: `700 18px ${SANS}`, color: BLUE }}>↓ — moving backward, v &lt; 0</span>
      </div>

      {/* equation */}
      <div style={{ position: 'absolute', left: 250, top: 572, opacity: win(lt, 12.0, 23.5, 0.5, 0.45), display: 'flex', alignItems: 'center', gap: 14 }}>
        <div style={{ borderLeft: `4px solid ${RED}`, padding: '10px 24px', background: '#fff', display: 'flex', alignItems: 'center' }}>
          <span style={{ font: `400 32px ${SERIF}`, color: INK }}><I>v</I>(<I>t</I>) =</span>
          <Frac n={<span style={{ fontStyle: 'italic' }}>dx</span>} d={<span style={{ fontStyle: 'italic' }}>dt</span>} color={INK} />
          <span style={{ font: `400 26px ${SERIF}`, color: INK2 }}>— the slope of the position graph, instant by instant.</span>
        </div>
      </div>
      <div style={{ position: 'absolute', left: 250, top: 572, opacity: win(lt, 24.0, 32, 0.5, 0.4) }}>
...
function SceneRecap() {
  ...
  const rows = [
    ...
    { from: 2.2, sym: <span><I>v</I> = <span style={{ display: 'inline-flex', verticalAlign: 'middle' }}><Frac n={<span style={{ fontStyle: 'italic' }}>dx</span>} d={<span style={{ fontStyle: 'italic' }}>dt</span>} /></span></span>, label: 'INSTANTANEOUS VELOCITY', desc: 'the slope of the x–t graph, right now' },
  ];
  const tHead = reveal(lt, 0.2, 0.6);
  return (
    <div style={{ position: 'absolute', inset: 0, opacity: fo, background: '#FFFFFF' }}>
      ...
      <div style={{ position: 'absolute', left: 110, top: 150 }}>
        {rows.map((r, i) => {
          ...
          return (
            <div key={i} style={{ display: 'flex', alignItems: 'baseline', gap: 36, padding: '21px 0', borderBottom: i < 2 ? `1px solid ${FAINT}` : 'none', opacity: rv.o, transform: `translateY(${rv.ty}px)` }}>
              <div style={{ width: 320, font: `400 46px ${SERIF}`, color: INK }}>{r.sym}</div>
              <div>
                <div style={{ font: `800 14px ${SANS}`, letterSpacing: '.16em', color: RED, marginBottom: 6 }}>{r.label}</div>
                <div style={{ font: `400 24px ${SERIF}`, color: INK2 }}>{r.desc}</div>
              </div>
            </div>
          );
        })}
      </div>
      <div style={{ position: 'absolute', left: 110, right: 110, top: 642, opacity: reveal(lt, 3.4, 0.7).o }}>
        <span style={{ font: `300 34px ${SANS}`, color: INK, letterSpacing: '-0.02em' }}>
...
function MotionScene() {
  return (
    <Stage width={1280} height={720} duration={172} background="#FFFFFF" persistKey="motion-line">
      ...
      <Sprite start={58.0} end={92.3}>  <SceneAvgVel /></Sprite>
      <Sprite start={92.0} end={128.3}> <SceneInstVel /></Sprite>
      <Sprite start={128.0} end={160.3}><SceneGraphRead /></Sprite>
      <Sprite start={160.0} end={172}>  <SceneRecap /></Sprite>
    </Stage>
  );
}
